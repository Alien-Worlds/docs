...
*** Note: It's important to ensure the sender is only from `uspts.worlds` to ensure that the custom logic could be faked from from an inpersonating user points account. ***
...
** Notice: there are 2 authorizations that are required in the action. **
