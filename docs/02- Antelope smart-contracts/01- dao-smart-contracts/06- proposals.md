...
### "Happy path" sequence for a worker proposal with straight approvals:

```mermaid
sequenceDiagram
autonumber
actor P as Worker/Proposer
actor C as DAO custodian
participant D as DAO Proposal Contract
participant E as Escrow contract
participant T as Token Treasury
P->>D: Create Proposal
activate D
C->>D: Vote on Proposal
P->>D: Start Work[if enough votes]
D->>E: Init escrow for proposal
activate E
T->>E: Transfer from Treasury
P->>D: Complete Work
C->>D: Vote on Proposal finish
P->>D: Finalise Proposal [if enough votes]
D->>E: Approve Escrow release
deactivate D
E->>P: Pay Worker
deactivate E
```
...
## Actions:
---
### Create proposal <BlockExplorerActionLinks contract="prop.worlds" action="createprop"/>
...
### Voting for a proposal to begin <BlockExplorerActionLinks contract="prop.worlds" action="voteprop"/>

Once a proposal has been created it would be in a state waiting for the current custodians to vote either ‘proposal_approve’ or ‘proposal\_deny’ for a proposal with the required number of votes and number of ‘yes’ votes to be configurable in the contract. At this time there may be refinements to the proposal with cancelling `cancel` of existing proposals and resubmitting changes based on feedback from the custodians until the proposals get to the ready and positively-voted-for position.

### Cancel a proposal before starting work <BlockExplorerActionLinks contract="prop.worlds" action="cancelprop"/>
...
### Start work on an accepted proposal <BlockExplorerActionLinks contract="prop.worlds" action="startwork"/>
...
### Cancel a proposal after starting work <BlockExplorerActionLinks contract="prop.worlds" action="cancelwip"/>
...
### Signal completion of work <BlockExplorerActionLinks contract="prop.worlds" action="completework"/>
...
### Voting for a proposal to complete <BlockExplorerActionLinks contract="prop.worlds" action="votepropfin"/>
...
### Dispute an unapproved proposal <BlockExplorerActionLinks contract="prop.worlds" action="dispute"/>
...
### Finalize a proposal for completed work <BlockExplorerActionLinks contract="prop.worlds" action="finalize"/>
...
### Contract configuration <BlockExplorerActionLinks contract="prop.worlds" action="updateconfig"/>
...
## Tables:
---
### proposals <BlockExplorerTableLinks contract="prop.worlds" table="proposals"/>

This table holds all the details about a given proposal scoped to the related DAO in the table. 

The fields included are:
* proposal_id: (name) - Proposal unique identifier
* proposer: (name) - The account name of the proposer
* arbitrator: (name) - The account name of the arbitrator being proposed. This should be a neutral entity both the DAO and worker trust to be fair.
* content_hash: (string) - a content hash to ensure the details of the proposals are maintained.
* proposal_pay: (extended_asset) - The amount of EOSIO based tokens that the proposer will get paid for the work.
* arbitrator_pay: (extended_asset) - The amount of EOSIO based tokens that the arbitrator will get paid if their services are required.
* state: (uint8_t) - The running state of the proposal
* expiry: (time_point_sec) - The date/time that the worker proposal will expire if it hasn't had sufficient approvals to start the work.
...
The fields included are:
* vote_id (uint64_t) - A unique identifier for the vote.
* voter (name) - The account for the voter
* proposal_id (optional name) - optional proposal that the vote is linked to if it's a direct vote on a proposal vs a category vote.
* category_id (optional uint64_t) - optional category that the vote is linked to if it's category vote vs a direct vote
* vote (optional name) - optional vote value if it's a vote vs delegating vote power to dalegatee.
...
The fields included are:
* proposal_threshold (uint8_t) - The number of approvals required for a proposal to startwork
* finalize_threshold (uint8_t) - The number of approvals required for a proposal to finalize payment
* approval_duration (uint32_t) - The duration in seconds before a proposal expires.