...
**Sidebar order is set by `sidebar_position` and `_category_.json`, not by renaming.** The `NN- ` filename prefixes are part of every existing URL, so inserting a section by renumbering directories would break inbound links. `docs/Architecture/` sits at position 2 via its `_category_.json`; the existing directories keep their names and get positions the same way.

**Sidebar is otherwise autogenerated** from the `docs/` directory tree (`sidebars.js`, `{type: 'autogenerated', dirName: '.'}`). Ordering and labels therefore come from filenames: the `NN- ` numeric prefixes (`02- Antelope smart-contracts/`, `03- mining.md`) _are_ the sort order. Renaming or renumbering a file moves it in the nav and changes its URL — check for inbound links first.
...
`docs/Architecture/` holds the narrative layer, read before the per-contract reference:

- `01- mental-model.md` — the system in one page, no action or table names. Explains the two primitives everything is built from: **deposits arrive as `transfer` notifications** (there is no deposit action anywhere, and the `memo` is effectively part of each contract's interface), and **contracts act on each other with inline actions** (so a mine action is atomic across four contracts).
- `02-` to `07-` — one page per subsystem: token and inflation, mining and rewards, planets and staking, DAO governance, NFTs and items, cross-chain and payouts.

Diagrams are hand-authored Mermaid, but the **edges come from `contract-graph.json`** rather than from memory. Regenerate the graph and re-read the `calls` and `notifiedBy` fields before redrawing anything. Deliberately hand-authored rather than generated: a complete 30-contract diagram is unreadable, and the value of a diagram is the editorial choice of what to leave out.

Two facts worth knowing when editing these pages:

- **The DAO layer resolves contract accounts at runtime** through the `index.worlds` directory, so several call edges are genuinely dynamic and cannot be drawn as fixed arrows. That is also the extension seam: register in the directory rather than patching code.
- **Vote weight is observed, not read.** The custodian contract never reads balances; the token contract sends `balanceobsv`, optionally via a `VOTE_WEIGHT` contract that computes its own weight and replies with `weightobsv`. `stkvt.worlds` is the shipped example.
